/*
 * TrivisionChoreo v7.12
 *
 * TWO-FACE WATERFALL — never reveals third prism face.
 * M1→M12 cascade CCW 120°, then M1→M12 cascade CW 120° back.
 * 0.75 RPM, S-curve ramps, seamless loop forever.
 */
...
void setup() {
  ...
  Serial.println(F("=== TrivisionChoreo v7.12 — TWO-FACE WATERFALL ==="));
  Serial.println(F("0.75 RPM, M1→M12 CCW, then M1→M12 CW, loop"));
  Serial.println();

  delay(3000);
}

// === TWO-FACE WATERFALL — loops forever ===
void loop() {
  // --- Waterfall CCW: face 1 → face 2 ---
  Serial.print(F("CCW: "));
