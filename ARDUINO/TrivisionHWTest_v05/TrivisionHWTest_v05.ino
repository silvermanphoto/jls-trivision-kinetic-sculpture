/*
 * TrivisionHWTest v05 — POST-REWIRE SMOKE TEST AND MOTOR IDENTIFICATION
 * ============================================================
 * Date: 2026-07-29
 *
 * PURPOSE
 ...
 *
 * BEFORE YOU RUN THIS — TURN THE CURRENT DIALS DOWN
 *   Twelve motors standing still with their dials at maximum ask about
 *   155 watts of a 156 watt supply, which makes the supply shut down and
 *   retry — the pulsing. Around 1 amp per driver puts the whole sculpture
 *   near 35 watts. Counter-clockwise on the small ILimit pot is less.
 *
 * SAFETY
...
unsigned int nudgeDownTable[NUDGE_RAMP_STEPS];

static void fillRamp(unsigned int *up, unsigned int *down, long n) {
  for (long i = 0; i < n; i++) {
    long denom = n * n;
    up[i] = RAMP_START_TK - (unsigned int)((i * i * (long)SPEED_RANGE_TK) / denom);
    long j = n - 1 - i;
    down[i] = RAMP_START_TK - (unsigned int)((j * j * (long)SPEED_RANGE_TK) / denom);
  }
}

void buildRampTables() {
  fillRamp(rampUpTable, rampDownTable, RAMP_STEPS);
  fillRamp(nudgeUpTable, nudgeDownTable, NUDGE_RAMP_STEPS);
}
  ...
  for (uint8_t m = 0; m < NUM_MOTORS; m++) {
    ...
    if (motor[m].tickCounter == 0) {
      ...
      *chStepPort[m] &= ~(1 << chStepBit[m]);
      ...
      long decelStart = motor[m].targetSteps - moveRampSteps;
      if (i < moveRampSteps) {
        motor[m].ticksToNext = useNudgeRamp ? nudgeUpTable[i] : rampUpTable[i];
      } else if (i >= decelStart) {
        long k = i - decelStart;
        motor[m].ticksToNext = useNudgeRamp ? nudgeDownTable[k] : rampDownTable[k];
      } else {
...
// first one you tested. A coil pair landed the other way round does this, and
// it is harmless once it is written down.
...
// === WHAT THE LAST MOVE LEFT BEHIND ===
// If a move is stopped part-way the prism is at an angle, which the rest of
// this sketch promises never to leave. These remember enough to either finish
// the turn or back it out.
int  partialCh = -1;
long partialDone = 0;
long partialLeft = 0;
uint8_t partialDir = CCW;
bool partialWasNudge = false;
...
// stops the motor. Returns true if the move finished, false if it was stopped.
bool waitMotor(int m) {
  bool stopped = false;
  ...
  if (stopped) {
    partialCh = m;
    partialDone = done;
    partialLeft = total - done;
    partialWasNudge = useNudgeRamp;
    Serial.println();
    Serial.print(F("  STOPPED. Channel "));
    Serial.print(m + 1);
    Serial.print(F(" got about "));
    Serial.print((int)((done * 120L) / STEPS_120));
    Serial.println(F(" degrees into the turn, so that prism is sitting at an angle."));
...
void nudge(int ch, uint8_t dir) {
  ...
  partialDir = dir;
  if (!waitMotorLaunch(ch, dir, STEPS_NUDGE, true)) return;
  delay(400);
  partialDir = (dir == CCW) ? CW : CCW;
  if (!waitMotorLaunch(ch, partialDir, STEPS_NUDGE, true)) return;
...
void faceTurn(int ch, uint8_t dir) {
  ...
  partialDir = dir;
  if (waitMotorLaunch(ch, dir, STEPS_120, false)) {
...
void printCorrectedTables() {
  ...
  Serial.println();
  if (anyReversed) {
    Serial.println(F("Some prisms turn the opposite way to the rest. That is"));
    Serial.println(F("harmless — the fourth block above cancels it out in software,"));
    Serial.println(F("so there is no need to open anything up and re-wire."));
  } else {
...
void printHelp() {
  ...
  Serial.println(F("  f            finish a turn that you stopped part-way"));
  ...
  Serial.println(F("do. The motor eases to a halt in about a second."));
  Serial.println();
...
void nudgeAll() {
  ...
  for (uint8_t c = 0; c < NUM_MOTORS; c++) {
    ...
    Serial.println(F(" ..."));
    partialDir = CCW;
    if (!waitMotorLaunch(c, CCW, STEPS_NUDGE, true)) return;
    delay(400);
    partialDir = CW;
    if (!waitMotorLaunch(c, CW, STEPS_NUDGE, true)) return;
    delay(700);
  }
  Serial.println();
...
void sweepAll() {
  ...
  for (uint8_t c = 0; c < NUM_MOTORS; c++) {
    ...
    Serial.println(F(" ..."));
    partialDir = CCW;
    if (!waitMotorLaunch(c, CCW, STEPS_120, false)) return;
    delay(1200);
  }
  Serial.println();
...
void resumePartial(bool finish) {
  if (partialCh < 0 || partialLeft <= 0) {
    Serial.println(F("  There is no half-finished turn to deal with."));
    Serial.println();
    return;
  }
  int ch = partialCh;
  long steps = finish ? partialLeft : partialDone;
  uint8_t dir = finish ? partialDir : ((partialDir == CCW) ? CW : CCW);
  bool wasNudge = partialWasNudge;
  if (steps <= 0) {
    Serial.println(F("  That prism never actually moved — nothing to undo."));
    Serial.println();
    partialCh = -1;
    return;
  }
  ...
  partialLeft = 0;
  partialDir = dir;
  if (waitMotorLaunch(ch, dir, steps, wasNudge)) {
...
void handleLine(char *s) {
  ...
  if (s[0] == 'n' || s[0] == 'N') { nudgeAll(); return; }
...
void setup() {
  ...
  Serial.println();
  Serial.println(F("============================================================"));
  Serial.println(F("  TrivisionHWTest v05 — SMOKE TEST AND MOTOR IDENTIFICATION"));
  ...
  Serial.println(F("  Unplug a motor, put 24V on, and measure DC volts between"));
  Serial.println(F("  the metal screw of that board's small dial and a ground"));
  Serial.println(F("  terminal. 0.76 volts is one amp per phase, which is ample"));
  Serial.println(F("  here. The dials do not read out, so this is the only way"));
  Serial.println(F("  to know. Cooler, quieter, and kinder to the supply."));
  printHelp();
}
